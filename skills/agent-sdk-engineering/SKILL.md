...
| Provider, embedding, vision, or semantic-analysis adapter | Provider adapter guide; `model.py`; `openai_compatible.py`; focused adapter tests | Inject endpoint, credentials, exact model, and image loader locally; never serialize them over MCP or read ambient secrets. Check exact model binding before network use. A provider-native tool continuation may receive only the current batch's bounded result and exact matching call IDs; it must not restore transcript replay. Vector ranking remains untrusted until frozen-source admission. Semantic analysis proposes only source-bound non-critical findings; Gate 1, designer approval, and deterministic checks retain authority. |
...
| Tool or execution backend | Architecture reference; `policy.py`; `tool_registry.py`; `artifacts.py`; supervisor tests | Add a named registered capability; never add generic shell or arbitrary network access. Mutating/process work requires typed approval. Default governed mutable drafts must preserve immutable run/node/task write occurrences. Artifact diffs require both authorized operands unless a declared current-task draft occurrence proves the exception. |
...
| Gate/profile/telemetry | `verification.py`; `profiler.py`; `telemetry.py`; `audit_log.py`; `errors.py` | Log reviewable public turns and result handles, never hidden reasoning or secrets. Verify complete chains, not only bounded display pages; disclose the verified count. Preserve bounded/redacted terminal failure codes/details. Mark absent measurements unavailable; do not infer them. |
...
| Specification graph/version lock | `dependency_graph.py`; `specification_gate.py`; `git_versioning.py`; versioning tests | Use sorted deterministic graph traversals. Missing-reference blast radius is transitive reverse reachability. Classify version bumps from persisted structured snapshots and dependency edges, never filename keywords. Refuse automatic classification of a legacy tag without a snapshot until an approved baseline exists. A variant base ref must resolve to the declared approved-tag commit and the record must retain that commit. |
| External framework or evaluator integration | `integrations/contracts.py`; target adapter; interoperability guide; boundary tests | Keep SDK authority local. Use an explicit sanitized projection, strict output reduction, digest-only receipt, explicit failure mode, lazy optional dependency, and host-selected durable receipt sink. Canonicalize snake/camel/kebab/case variants before forbidden-key checks. No raw transcript/state, credentials, approval token, capability grant, or tool bypass may cross the boundary. |
