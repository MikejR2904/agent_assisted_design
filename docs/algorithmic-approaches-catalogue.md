...
| Episode compaction | Exact additive PCKP by default; PASK as opt-in diversity heuristic | Maximize dependency-closed static integer utility under a budget; dynamic PASK is labelled separately | Open/protected/manifest-incomplete/required prerequisites cannot be compacted | Exponential worst case for general PCKP; forest fast path is pseudo-polynomial | Exact/default and heuristic modes implemented |
