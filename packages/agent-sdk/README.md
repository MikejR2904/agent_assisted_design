...
The [P0 trust-guarantee repair record](../../docs/p0-trust-guarantee-repair.md) documents the 0.16.0 provenance, artifact-access, full-ledger verification, typed-failure, worktree-base, interop-key, bounded-regex, and portable-supervisor repairs, including their evidence and limitations.

The [update-audit remediation record](../../docs/update-audit-1e5c61e-remediation.md) documents the post-`1e5c61e` correctness review: recovery-safe split persistence, interprocess audit append serialization, lossless schema validation cache behavior, portable legacy artifact recovery, shared atomic publication, bounded default DAG compaction, and the expanded boundary-regression suite.

The [provider adapters and semantic Gate 1 guide](../../docs/provider-adapters-and-semantic-gate.md) documents the optional host-injected OpenAI-compatible chat, embedding, vision, and semantic-gap proposal adapters; their bounded provider-native tool continuation; frozen-source admission rules; and the distinction between offline contract validation and a live provider deployment.
...
| `tool_registry.py`, `policy.py`, `approvals.py`, `supervisor.py` | Closed capabilities, path containment, typed approval, two-sided artifact-diff authorization, immutable current-task draft proof, and registered-command supervision. Process outcomes are mapped deterministically: timeout, cancellation, resource-limit, and non-zero exit become typed tool failures rather than successful payloads. | Tool/hook and watchdog boundary.[18] |
| `telemetry.py`, `metrics.py`, `base_agent.py`, `controller_runtime.py` | SQLite-backed append-only event ledger, standard metric catalogue, explicit availability states, complete per-run hash-chain verification, aggregate trace reports, typed terminal failures, BaseAgent wall-clock watchdogs, and controller/graph lifecycle telemetry. The ledger records structured outcomes and evidence pointers, not hidden model reasoning. | Watchdog authority and accountability/provenance requirements.[18] [22] [23] |
| `audit_log.py` | Bounded review pages over redacted, hash-linked JSONL interaction records, with complete-chain verification and disclosed verified/rendered counts. Provider-visible structured turns, tool call/result evidence, verification, escalation, and terminal outcomes are review evidence; the log is never model working memory. | The proposal requires traceability of prompts, outputs, corrections, and PPA deltas; structural checkpoints omit raw transcripts.[19] [22] [25] |
| `core_tools.py`, `tool_registry.py` | Typed run-root-contained file/evidence tools, immutable artifact write occurrences, bounded result-handle retrieval, deadline- and byte-bounded isolated regex grep, untrusted public-web research, notebook JSON edits, human questions, and named registered-command dispatch. Generic shell, local-network web access, and runtime-discovered arbitrary tools are absent. | OpenHarness's typed-tool, read-only/approval, and untrusted-web patterns, adapted to the system-design capability/watchdog boundary.[18] [26] [29] |
...
Mixed documents are processed text-first. Image nodes retain neighboring text and are routed through a provider-neutral `VisionAdapter`. `ScriptedVisionAdapter` supports controlled tests and `UnconfiguredVisionAdapter` returns `review-required`. `OpenAICompatibleVisionAdapter` is an optional host-injected implementation: a host supplies the endpoint, exact model, credentials, and bounded image loader; `SourceVerifiedImageLoader` covers only standalone frozen PNG/JPEG sources. A valid high-confidence typed proposal is accepted after at most three attempts.[8] [9]
...
`ProcessSupervisor` is a tool-layer watchdog, not a generic shell. A closed `CommandTemplate` controls the executable, working directory, wall-clock deadline, bounded output capture, optional POSIX CPU/address-space/file-size limits, and retry eligibility. POSIX uses a new session/process group with `SIGTERM` then `SIGKILL`; Windows uses `CREATE_NEW_PROCESS_GROUP` and `taskkill /T /F` through an argument vector. The record names the PID/PGID where applicable, duration, exit kind/code/signal, termination path, output totals/truncation, and enforced or unsupported resource controls. The Windows branch is not yet empirically validated by a Windows CI run. The runtime does **not** claim cgroup isolation; the current portable implementation explicitly reports only limits it was able to apply.[18]
...
`TelemetryStore` creates `.agent-telemetry/telemetry.sqlite3` under the configured run root. Every event carries ordered run/task/controller/agent correlation, UTC and monotonic timestamps, actor/authority, status, evidence links, a payload hash, and the previous event hash. Full-chain verification pages to completion; reports disclose `event_count` and `verified_event_count` rather than representing a 1,000-record read page as the whole run. `STANDARD_METRIC_DEFINITIONS` automatically registers attempt/recovery/tool/verification/escalation/context/watchdog/profile metrics and the proposal’s research metric definitions. The report aggregates only available observations by each registered formula; it preserves unavailable values and their reasons instead of substituting zero. Provider input/output/cache/reasoning/context-window token counters are recorded only from `ModelTurnResponse.usage`; the deterministic context estimate and the provider counters are explicitly labelled differently.[22] [23]

`AuditTranscriptStore` writes `.agent-audit-logs/<run>.jsonl` and can render a review-only Markdown transcript. It records bounded, redacted task instructions, provider-visible structured model turns, tool arguments/results or journal handles, verification, escalation, and terminal/profile events. Complete-chain verification is independent of the bounded rendered page and discloses both counts. Secret-bearing keys are redacted and hidden-reasoning fields are rejected. Audit history is not replayed to the model and checkpoints remain structural, so tracing does not reintroduce context growth.[19] [25]

`core_tool_definitions()` supplies typed opt-in declarations for `read_file`, `glob`, `grep`, `read_artifact`, `grep_artifact`, `get_tool_result`, `write_draft`, `edit_draft`, `notebook_edit`, `diff_declared_artifacts`, `web_fetch`, `web_search`, `sleep`, `brief`, `ask_human_question`, and `run_registered_command`. `HarnessToolRegistry` applies the existing capability/approval policy before `CoreToolDispatcher` runs them. Default governed mutable writes retain immutable run/node/task occurrences; artifact diffs require both authorized operands unless a declared current-task output occurrence proves the draft. Files cannot escape the run root; model-facing grep has a child-process deadline plus file/byte budgets; web fetch rejects non-HTTP(S), loopback, private, link-local, multicast, reserved, and redirected-to-private targets; every web result is marked untrusted; and process execution accepts only a named `CommandTemplate`, never a raw command string.[18] [26] [29]
...
`CompactionStrategy.EXACT_PCKP` is the default retention selector. It optimizes an additive static integer utility under immediate prerequisite constraints, uses a deterministic branch-and-bound search with rational upper bounds for a general dependency DAG, and uses a capacity-indexed dynamic-programming path only for verified rooted forests. The policy bounds general-DAG branch expansion (`exact_max_branch_nodes`, default 50,000); a cap returns an explicit feasible `BEST_EFFORT` certificate rather than claiming optimality, while the rooted-forest dynamic-programming path remains exact. A solver dossier records the problem hash, selected closure, status, node count, bound, and certificate. `CompactionStrategy.PASK` remains an explicit deterministic diversity heuristic; `GREEDY_BASELINE` remains for controlled comparison. Protected/open/manifest-incomplete dependency closures are never silently dropped. See the [P0–P2 implementation record](../../docs/p0-p2-algorithm-implementation.md), [context-compaction/coordination explainer](../../docs/context-compaction-and-coordination.md), and [update-audit remediation record](../../docs/update-audit-1e5c61e-remediation.md).[27] [28]

`ProviderContinuation` and `ModelTurnResponse` provide an opaque carrier for provider-native response, session, or compaction state. `OpenAICompatibleAgentModel` is an optional host-injected Chat Completions adapter that checks the exact declared provider/model binding before network use. If that protocol issues a function call, `ProviderToolResultConsumer` receives only the current batch’s canonical, preview-bounded result and validates matching call IDs before resolving the native continuation. Ordinary next-turn context remains state-first: the full result stays in the journal and is not replayed as an observation or transcript. No default provider, credential lookup, live model integration, or model-selection policy is built into the SDK.[30]
...
The stable consumer entry point is `import agent_sdk`. A host application supplies an `AgentDefinition`, a scoped task, an `AgentModel` adapter for its selected provider, an optional declared-tool executor, and optional local extensions. `ScriptedModel` is deliberately a test adapter; it is not a model provider. `OpenAICompatibleAgentModel`, `OpenAICompatibleEmbeddingProvider`, `OpenAICompatibleVisionAdapter`, and `OpenAICompatibleSemanticGapAnalyzer` are available as optional adapters, but their endpoint, credentials, exact model ID, source/image loader, and deployment controls remain host-owned.[30]
...
The package is **not yet an autonomous RTL-to-GDSII flow**. Tests use `ScriptedModel`, `ScriptedVisionAdapter`, controlled source files, controlled subprocesses, and in-memory OpenAI-compatible protocol transports. No live LLM or multimodal provider, Verilator, Yosys, OpenROAD, or OpenSTA binary has been invoked. Registered EDA commands remain disabled until the project specifies the executable, environment, resource policy, and manifest contract.

The present parser records image positions and associated context but does not persist extracted binary assets or claim to understand any diagram without an accepted vision-adapter proposal. SystemRDL, SDC, and UPF are source-preserving line/statement inputs in this increment; semantic compilers are not yet bound. Gate 1 deterministically checks absence, traceability, and verifiability. A host may now submit or produce typed ambiguity, semantic-inconsistency, and unstated-assumption findings, but the gate admits them only when requirement IDs and exact source references bind to the frozen specification. They remain non-critical proposals rather than proven facts, cannot soft-lock a specification, and require designer review.[30]
...
[29]: https://cwe.mitre.org/data/definitions/1333.html "CWE-1333: Inefficient Regular Expression Complexity; non-backtracking engines, execution limits, and input limits"
[30]: ../../docs/provider-adapters-and-semantic-gate.md "Host-injected provider adapters, bounded continuation, and source-bound semantic Gap 1 admission"
