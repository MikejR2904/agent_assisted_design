...
STANDARD_METRIC_DEFINITIONS: tuple[MetricDefinition, ...] = (
    ...
    _definition(
        ...
        "Grounded retrieval candidate count",
        "count",
        "lower",
        "count(retrieval.completed candidates)",
        aggregation="sum",
        missing_data_rule="observed when a retrieval telemetry sink is configured",
        source_description="GroundedRetrievalService",
    ),
    _definition(
        "retrieval.cache_hit_count",
        "Grounded retrieval cache-hit count",
        "count",
        "higher",
        "1 when retrieval.completed status=cache-hit; otherwise 0",
        aggregation="sum",
        missing_data_rule="observed when a retrieval telemetry sink is configured",
        source_description="GroundedRetrievalService",
    ),
